...
/**
 * Compact shift card for the Day Action Modal.
 * Displays shift name (max 50 chars, ellipsis), start time (HH:mm), end time (HH:mm).
        ...
        Row(
            ...
            Box(
                modifier = Modifier
                    ...
                    .fillMaxHeight()
                    .background(
                        color = parseHexColorSafe(event.backgroundColor),
...
@Composable
private fun DayActionShiftCardPreview() {
    PlanixorTheme {
        DayActionShiftCard(
            event = CalendarEventDisplay(
                id = "1",
                ...
                syncedAt = null,
                isDeleted = false,
                name = "Morning Shift with a very long name that exceeds fifty characters limit",
                icon = "☀️",
