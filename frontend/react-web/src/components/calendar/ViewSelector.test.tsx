...
describe('ViewSelector', () => {
  ...
  it('should render Day, Week, Month, Year labels from i18n', () => {
    renderViewSelector();
  ...
  it('should apply active styling class to the selected tab', () => {
    useCalendarStore.setState({ activeView: 'year' });
    renderViewSelector();

    const yearTab = screen.getByRole('tab', { name: 'Year' });
    expect(yearTab.className).toMatch(/tabActive/);

    const dayTab = screen.getByRole('tab', { name: 'Day' });
    expect(dayTab.className).not.toMatch(/tabActive/);
  });
});
