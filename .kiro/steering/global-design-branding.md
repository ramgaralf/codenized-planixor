...
- **No FAB (Floating Action Button) on web** — creation actions are placed in the top bar (HeaderBar), not as floating buttons. The "+" button appears conditionally based on the current page route (Calendar → New Event, Shifts → New Shift, etc.)
- **No FAB on Android shift screens** — the "New Shift" action is in the global top bar, not a FAB. Only Calendar retains the FAB for new events. Sub-screens (forms) show a back arrow + title in the top bar instead of the global brand bar.
- **Color palette**: The Predefined_Palette has been expanded to 9 families × 5 shades (45 colors total). Colors are displayed with recommended shades highlighted based on theme (dark mode → lighter shades recommended, light mode → darker shades recommended). Non-recommended shades show at 50% opacity but remain selectable.
- **Emoji picker**: Both platforms use a rich emoji picker with categories (Faces, Gestures, Nature, Animals, Food, Sports, Travel, Objects, Symbols), scrollable grid, and ~40+ emojis per category. Web uses `emoji-picker-react` library; Android uses a custom Compose dialog with category tabs.
- **Confirmation dialogs** must include the name of the affected item (e.g., "Delete 'Morning Shift'?") for clarity.
