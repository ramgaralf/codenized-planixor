...
- **No FAB (Floating Action Button) on web** — creation actions are placed in the top bar (HeaderBar), not as floating buttons. The "+" button appears conditionally based on the current page route (Calendar → New Event, Shifts → New Shift, Reminders → New Reminder)
- **No FAB on Android shift/reminder screens** — the "New Shift" and "New Reminder" actions are in the global top bar, not a FAB. Only Calendar retains the FAB for new events. Sub-screens (forms) show a back arrow + title in the top bar instead of the global brand bar.
- **Color picker**: The Predefined_Palette has 9 families × 5 shades (45 colors total). On both platforms, the color picker uses a **button + dropdown/dialog** pattern: a circular button shows the selected color (or 🎨 placeholder if empty), clicking it opens a dropdown (web) or dialog (Android) with the full color grid. Non-recommended shades show at 50% opacity but remain selectable. Theme-aware recommendations: dark mode → lighter shades (indices 0-2), light mode → darker shades (indices 2-4).
- **Emoji picker**: Both platforms use the same pattern: a button showing the selected emoji (or ➕ placeholder), clicking opens the picker inline (web: `emoji-picker-react` with full width, 350px height, search support) or as a dialog (Android: custom Compose dialog with category tabs, scrollable grid, ~40+ emojis per category). Closes on selection or click outside.
- **Activate/Deactivate toggle icon**: Both platforms use **Pause** icon (web: `lucide-react` Pause / Android: `Icons.Outlined.Pause`) when the item is active (action = deactivate), and **Play** icon (web: `lucide-react` Play / Android: `Icons.Outlined.PlayArrow`) when the item is inactive (action = activate). Do NOT use Power icon.
- **Confirmation dialogs** must include the name of the affected item (e.g., "Delete 'Morning Shift'?", "Are you sure you want to deactivate 'Take Medicine'?") for clarity.
- **Page titles only in top bar** — individual pages (ReportsPage, SettingsPage, etc.) do NOT render their own title heading. The global top bar handles all page titles.
- **"Today" button in all calendar navigators**: Every view mode's date navigator (Day, Week, Month, Year) includes a "Today/Hoy" button that resets navigation to the current date. The button is right-aligned within the navigator bar, styled as a small outlined text button with `primary-blue` text. Applies to both web and Android.
