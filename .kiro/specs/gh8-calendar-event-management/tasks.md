...
- [ ] 1. Migrate data model and validation (React Web)
  - [ ] 1.1 Update `models.ts`: rename `day` → `startDay`, add `endDay`, add `totalHours`, update notes max to 250
  ...
  - [ ] 1.2 Update `constants.ts`: MAX_NOTES_LENGTH = 250, add new i18n keys for new validation errors
  ...
  - [ ] 1.3 Rewrite `validation.ts`: remove `validateTimeRange`, add new validation functions
  ...
  - [ ] 1.4 Update Dexie schema to v5 (rename `day` index to `startDay`, add `endDay`, add `totalHours`)
  ...
  - [ ] 1.5 Update `calendarEventService.ts`: migrate CRUD and query logic to new model
  ...
  - [ ] 1.6 Write/update property tests for new validation functions
...
- [ ] 2. Migrate form and UI (React Web)
  - [ ] 2.1 Update `useEventForm.ts`: replace `day` with `startDay`/`endDay`, add conditional time editability, auto-populate shift times, compute totalHours, crossing midnight auto-set endDay
  ...
  - [ ] 2.2 Update `EventForm.tsx`: add second datepicker for endDay, conditional time field editability, display totalHours (read-only), update notes maxLength to 250
  ...
  - [ ] 2.3 Update `EventDetailPage.tsx` for new fields
  ...
  - [ ] 2.4 Update view components (DayView, WeekView, MonthView, YearView) to use `startDay`/`endDay` for filtering (range intersection)
  ...
  - [ ] 2.5 Update `useEventFiltering.ts` for range intersection logic
  ...
  - [ ] 2.6 Update i18n files (en.json, es.json) with new keys
  ...
  - [ ] 2.7 Update/fix all existing tests to work with new model
...
- [ ] 3. Migrate sync (React Web)
  - [ ] 3.1 Update `calendarEventSync.ts` DTOs and logic with `startDay`, `endDay`, `totalHours`
    - Update `CalendarEventSyncRecord` interface: rename `day` → `startDay`, add `endDay`, add `totalHours`
...
- [ ] 4. Migrate backend
  - [ ] 4.1 Update `CalendarEvent.cs` entity: rename `Day` → `StartDay`, add `EndDay`, add `TotalHours`, update Notes max 250
  ...
  - [ ] 4.2 Update EF Core configuration + add migration
  ...
  - [ ] 4.3 Update sync DTOs with new fields
    - Update `CalendarEventSyncRecord`: rename `Day` → `StartDay`, add `EndDay` (string, ISO date), add `TotalHours` (int)
    - Update `CalendarEventSyncPushRequest` and `CalendarEventSyncPullResponse` accordingly
    - _Requirements: 10.1, 10.9_

  - [ ] 4.4 Update `CalendarEventSyncPushService` validation (remove EndTime > StartTime CHECK, add EndDay >= StartDay)
...
- [ ] 5. Migrate Android
  - [ ] 5.1 Update `CalendarEventEntity.kt`, `CalendarEvent.kt`, `CalendarEventDisplay.kt` with startDay/endDay/totalHours
    - Rename `day` → `startDay` in entity, domain model, and display model
    - Add `endDay: String` field (ISO date YYYY-MM-DD)
    - Add `totalHours: Int` field (minutes, computed)
    - Update notes max length reference to 250
    - _Requirements: 11.1_

  - [ ] 5.2 Update `CalendarEventDao.kt` queries
  ...
  - [ ] 5.3 Update `CalendarEventRepository.kt` CRUD + validation
  ...
  - [ ] 5.4 Update `CalendarEventValidation.kt` with new rules
  ...
  - [ ] 5.5 Update Room migration
    - Add Room migration (increment version): rename `day` → `startDay`, add `endDay` TEXT NOT NULL, add `totalHours` INTEGER NOT NULL
    - Clear existing calendar event data (no user data in deployed environments)
    - Update indices
    - _Requirements: 11.1_

  - [ ] 5.6 Update `CalendarViewModel.kt` form state
  ...
  - [ ] 5.7 Update `EventFormScreen.kt` UI (second datepicker, conditional time fields, totalHours display)
  ...
  - [ ] 5.8 Update view composables for range intersection filtering
    - DayView: show events where `startDay <= currentDay <= endDay`
  ...
  - [ ] 5.9 Update sync adapter DTOs
    - Update `CalendarEventSyncRecord` data class: rename `day` → `startDay`, add `endDay`, add `totalHours`
    - Update serialization/deserialization logic
    - _Requirements: 10.1, 10.9_

  - [ ] 5.10 Update Android unit tests
