...
7. THE Shift_Table SHALL display one row per shift type showing: the shift's emoji icon, shift name, number of calendar events of that type, and total hours for that type formatted as "{X}h {Y}m", followed by a final summary row showing the label "Total", total event count, and the grand total of all shift hours for the month formatted as "{X}h {Y}m".
...
7. THE Reminder_Table SHALL display one row per reminder type showing the reminder emoji icon, reminder name, number of calendar events of that type, and total hours formatted as "{X}h {Y}m", ordered by total hours descending, followed by a final summary row showing the label "Total", total event count, and the grand total of all reminder hours for the month formatted as "{X}h {Y}m".
...
5. WHEN annual working hours ARE configured for the selected year, THE Shift_Donut_Chart SHALL display the percentage of each shift type where 100% represents the total configured working hours for that year (individual shift type percentages may exceed 100% if actual hours surpass configured hours), with each percentage rounded to one decimal place, and SHALL display the total combined shift hours versus the configured working hours in the center of the donut formatted in two lines as "{actual hours}h {actual minutes}m /\n{configured hours}h {configured minutes}m".
6. WHEN annual working hours are NOT configured, THE Shift_Table SHALL display one row per shift type showing the shift name, icon, number of calendar events of that type, and total hours for that type formatted as "{X}h {Y}m", ordered from highest total hours to lowest, and a final row showing the grand total event count and total of all shift hours for the year formatted as "{X}h {Y}m".
7. WHEN annual working hours ARE configured, THE Shift_Table SHALL display one row per shift type showing the shift name, icon, number of calendar events of that type, and total hours for that type formatted as "{X}h {Y}m", ordered from highest total hours to lowest, followed by a row showing the grand total event count and total of all shift hours formatted as "{X}h {Y}m", a row showing the configured annual working hours formatted as "{X}h {Y}m", and a row showing the difference formatted as "+{X}h {Y}m" or "-{X}h {Y}m"; IF total shift hours are greater than or equal to the configured hours, THEN the difference row text SHALL be rendered in green (#10B981); IF total shift hours are less than the configured hours, THEN the difference row text SHALL be rendered in red.
...
6. THE Reminder_Table SHALL display one row per reminder type showing the reminder emoji icon, reminder name, number of calendar events of that type, and total hours for that type formatted as "{X}h {Y}m", rows sorted in descending order by total hours, and a final row showing the grand total event count and total of all reminder hours for the year.
