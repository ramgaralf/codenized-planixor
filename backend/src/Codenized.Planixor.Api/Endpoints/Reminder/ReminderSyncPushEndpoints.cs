...
internal static class ReminderSyncPushEndpoints
{
    ...
    public static RouteGroupBuilder MapReminderSyncPushEndpoint(this RouteGroupBuilder group)
    {
        group.MapEndpoint<GenericResponse<ReminderSyncPushResponse>>(
            HttpMethods.Post,
            "/push",
            async (ReminderSyncPushRequest request, IController<ReminderSyncPushRequest, ReminderSyncPushResponse> controller, ISecurityService securityService, CancellationToken cancellationToken) =>
            {
                ...
                GenericResponse<ReminderSyncPushResponse> result = await controller.Handle(request, cancellationToken);
