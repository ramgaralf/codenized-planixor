...
internal static class AnnualHoursConfigSyncPushEndpoints
{
    ...
    public static RouteGroupBuilder MapAnnualHoursConfigSyncPushEndpoint(this RouteGroupBuilder group)
    {
        group.MapEndpoint<GenericResponse<AnnualHoursConfigSyncPushResponse>>(
            HttpMethods.Post,
            "/push",
            async (AnnualHoursConfigSyncPushRequest request, ISecurityService securityService, IController<AnnualHoursConfigSyncPushRequest, AnnualHoursConfigSyncPushResponse> controller, CancellationToken cancellationToken) =>
            {
                ...
                GenericResponse<AnnualHoursConfigSyncPushResponse> result = await controller.Handle(request, cancellationToken);
